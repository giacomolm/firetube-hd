define(["jquery", "models/Video"],
	...
	Utils.display = function(id, collection){
		...
		oReq.open("get", 'http://hdeos.pythonanywhere.com/api/info?url=http://www.youtube.com/watch?v='+id+'', true);
		oReq.send();							
	};
	...
	Utils.displayOnly = function(id, collection){
		...
		oReq.onreadystatechange = function (event) {
		    ...
		    if (xhr.readyState === 4 && xhr.status === 200 ) {
			    var videos = (JSON.parse(results)).info.formats;
          var urls = new Object();
          //setting the property directly in the collection
          temp_collection.thumbnail = (JSON.parse(results)).info.thumbnail;
			    if(videos && videos[0]){
		...
		oReq.open("get", 'http://hdeos.pythonanywhere.com/api/info?url='+id, true);
		oReq.send();							
	};
		...
		if(query!=""){			
			...
			$.get("http://suggestqueries.google.com/complete/search?callback=?",
				...
				function(data) {
					...
					// non voglio fare pi� scroll se la ricerca non ha dato esito
				});
		}
	};
